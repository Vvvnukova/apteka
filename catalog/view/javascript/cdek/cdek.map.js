function showMap(element){
	...
	var input = $('input[name=shipping_method][value^=\'cdek.pvz_' + tariff_id + '_\']');
	...
	$.ajax({
		...
		success: function(json) {
			...
			html  = '<div id="modal-cdek-map" class="modal">';
			html += '  <div class="modal-dialog">';
			html += '    <div class="modal-content">';
			html += '      <div class="modal-header">';
			html += '        <h4 class="modal-title">Выберите адрес</h4>';
			...
			$('#modal-cdek-map').modal('show');

			if (markers && markers.length > 0) {

				$('#modal-cdek-map .selectpicker').selectpicker({
					liveSearch: true,
					maxOptions: 1
				});
	...
	ymaps.ready(function() {
		...
		var balloon = ymaps.templateLayoutFactory.createClass(
			...
			'<div class="map-info--footer">' +
				'<a class="btn btn-primary">Выбрать</a>' +
			'</div>'
		);
