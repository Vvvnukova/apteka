...
class ModelShippingCdek extends Model
{
	...
	public function getQuote($address)
	{
		...
		if ($this->isAdmin()) {
			...
			$this->config->set('cdek_view_type', 'default');
								...
								foreach ($results as $shipping_info) {
									...
									if (array_key_exists($shipping_info['tariffId'], $cdek_tariff_list)) {
										...
										if ($this->config->get('config_currency') == $shipping_info['currency']) {
											$_price = $shipping_info['priceByCurrency'];
										} else {
											$_price = $this->currency->convert($shipping_info['priceByCurrency'], $shipping_info['currency'], $this->config->get('config_currency'));
										}

										$price = $shipping_price = $_price;
											...
											if ($this->config->get('cdek_show_pvz')) {
												...
												if (in_array($this->config->get('cdek_view_type'), array('group', 'map'))) {
													...
													if ('map' == $this->config->get('cdek_view_type')) {
														...
														$quote_description = $this->getHTML($shipping_info['tariffId'], $pvz_list);
													}
	...
	private function getMap($data)
	{
		...
		if ($first_call) {
			...
			$html .= '<script src="/catalog/view/javascript/cdek/bootstrap-select.min.js" type="text/javascript"></script>';
            $html .= '<link href="/catalog/view/theme/default/stylesheet/cdek/bootstrap-select.min.css" rel="stylesheet" type="text/css">';
		...
		$html .= '<div class="cdek-container" style="margin-top:5px;margin-bottom:15px">';
	...
	private function getHTML($tariff_id, $pvz_list)
	{
		static $first_call = true;
		...
		if (count($pvz_list) > 1) {
			...
			$html .= '	<select onChange="event.preventDefault(); event.stopPropagation(); changePVZ(this);" class="cdek-pvz-list form-control" data-id="' . $tariff_id . '" data-active="' . $active_pvz . '" style="margin-top:5px;margin-bottom:5px">';
	...
	private function getResult(CalculatePriceDeliveryCdek &$calc, $total = 0)
	{
		...
		if (true === $calc->calculate()) {
			...
			if (!$this->config->get('cdek_cache_on_delivery') ||
				(!isset($response['result']['cashOnDelivery']) || !(float)$response['result']['cashOnDelivery']) ||
				($this->config->get('cdek_cache_on_delivery') && (float)$response['result']['cashOnDelivery'] && $total >= (float)$response['result']['cashOnDelivery'])) {
			...
			if (isset($pvz_list_data->Pvz)) {
				...
				foreach ($pvz_list_data->Pvz as $pvz_info) {
					...
					if (isset($pvz_info->WeightLimit)) {
						...
						if ($min_weight == $max_weight && 0 == $max_weight) {
							continue;
						}
					...
					$pvz_address = 'г. ' . $pvz_info['City'] . ', ' . $this->mb_ucfirst($pvz_info['Address'], 'UTF-8', true) . '.';
					...
					$pvz_list[$type][$city_id][$key] = array(
						'address'		=> $this->mb_ucfirst($pvz_info['Address'], 'UTF-8', true),
						'address_full'	=> $pvz_address,
						'address_short'	=> 'г. ' . $pvz_info['City'] . ', ' . $this->mb_ucfirst($pvz_info['Address'], 'UTF-8', true),
	...
	{
		return !empty($this->request->get['route']) && false !== strpos($this->request->get['route'], 'api/');
	}
...
class parser_xml implements response_parser
{
	public static function convert($data = '')
	{
		return (strpos($data, '<?xml') !== FALSE) ? new SimpleXMLElement($data) : '';
	}
}
